...
# 1. Sample randomly with replacement 50 curves among the 70 curves of the 
# Training Dataset. Build a table containing these randomly selected curves and 
...
ct_10=rep(0,10)
for (i in 1:10){
  Training_Base_i=Main_Training_Base[sample(nrow(Main_Training_Base), 50, 
                                          replace=TRUE), ]
  #TODO
}
...
#Exercise 3.5 Accuracy of the adjusted model

# 1. Choose one or several indicators quantifying the goodness of fit of the 
# adjusted model. Justify your choice ;

#According to this resource: 
#https://www.r-bloggers.com/2015/01/goodness-of-fit-test-in-r/
#There are three main tests for goodness of fit.
# - Chi Square test
# - Kolmogorov–Smirnov test
# - Cramér–von Mises criterion
#They also happen to have dedicated packages in R, making their use relevant
#in that code.

# 2. By using the chosen indicators, compute the accuracy of the model on the 
# Training Dataset ;
#TODO : vérifier les résultats
#Chi Square test
...
# 1. Find an optimization algorithm based on a gradient descent method. Briefly 
# explain the functioning of this algorithm ;

# http://eric.univ-lyon2.fr/~ricco/tanagra/fichiers/fr_Tanagra_Gradient_Descent_R.pdf
#install.packages("gradDescent")
library(gradDescent)
?gradDescentR.learn
#With the default parameters, using learningMethod = "GD" (so the Gradient 
#Descent method), it does apply the Gradient Descent algorithm, which is
#starting from an initial point that we call X0 here):
#1 - compute the gradient of f(Xk), called delta (f being the function for which 
#we want to find an optimal value, and Xk the current point)
#2 - if the norm of that gradient is smaller than a set treshold, we stop the 
#computation here
#3 - then you compute the new point: Xk+1 = Xk + alpha * delta(f(Xk)), where
#alpha is called the learning rate. It is the parameter that allows you to move
#faster or slower along the curve. It is often set at the beginning of the 
#algorithm, but some functions can update it at each iteration. In 
#gradDescentR.learn, it is set by default at 0.1, but can be updated with the
#parameter "control".
#Once you have your new point, you start again, until you reach the condition
#of the second step.


# 2. Use this algorithm to adjust the parameters a, b and c from the training 
# dataset by initializing the three parameters at 0. What are the obtained 
# values of the parameters ?
a0=0
b0=0
c0=0

# 3. Launch the adjustment of the parameters by testing different initializations 
# of the parameters a, b and c (5 tests). Present the results and comment them ;
at_5=rep(0,5)
bt_5=rep(0,5)
ct_5=rep(0,5)
for (i in 1:5){
...
# 5. Fix the parameters a, b and c at their average value previously calculated. 
# By using the chosen indicators, compute the accuracy of the model on the 
# training dataset and on the test dataset ;


# 6. Compare the algorithm DIRECT and the algorithm based on a gradient descent
# method. Which algorithm seems most suitable ? In which cases and why ?
